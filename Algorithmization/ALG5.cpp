...
tuple<map<string, Coord>, vector<EdgeItem>> read_graphml(const string& file_path) {
    ...
    for (tinyxml2::XMLElement* edge = graph->FirstChildElement("edge"); edge; edge = edge->NextSiblingElement("edge")) {
        ...
        for (tinyxml2::XMLElement* data = edge->FirstChildElement("data"); data; data = data->NextSiblingElement("data")) {
            ...
            // В данном GraphML-файле название улицы хранится в ключе d18.
...
pair<int, string> find_street_index(const vector<EdgeItem>& edges, const string& street_name_query) {
    for (size_t i = 0; i < edges.size(); i++) {
        ...
        if (!name.empty()) {
            ...
            string lc_name_utf8 = convert_cp1251_to_utf8(lc_name);
            string query_utf8 = convert_cp1251_to_utf8(query);

            if (lc_name_utf8 == query_utf8 || lc_name_utf8.find(query_utf8) != string::npos) {
...
// Функция отображает дорожную сеть и найденный кратчайший маршрут.
...
// dpi: разрешение изображения (не используется в данном варианте, так как SFML сохраняет по пикелям)
...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("bratislava.graphml");
    // print_edges(edges);

    if (edges.empty()) {
        std::cout << "Файл графа не прочитан или в нем нет ребер." << std::endl;
        std::cout << "Проверьте, что файл bratislava.graphml лежит рядом с exe." << std::endl;
        return 1;
    }
    ...
    string start_street_query = "Einsteinova";      // Начальная улица маршрута
    string end_street_query = "Gagarinova";         // Конечная улица маршрута
    ...
    if (start_index == -1 || end_index == -1) {
        ...
        std::cout << "Включите строку print_edges(edges), посмотрите названия улиц и замените start_street_query/end_street_query." << std::endl;
    }
    else {
        // 4. Получаем вершины начала и конца маршрута:
        // Для начальной улицы берём первую координату ребра, для конечной — вторую.
        ...
        double dijkstra_time_ms = std::chrono::duration<double, std::milli>(end_time - start_time).count();
        ...
        if (path.empty()) {
            ...
            std::cout << "Время работы алгоритма: " << dijkstra_time_ms << " мс" << std::endl;
